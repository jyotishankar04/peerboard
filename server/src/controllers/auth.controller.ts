...
export class AuthController {
  ...
  public async register(req: Request, res: Response, next: NextFunction) {
    const { data: body, success: validationSuccess } =
      await registerSchema.safeParseAsync(req.body);
  ...
  public async verifyOtp(req: Request, res: Response, next: NextFunction) {
    ...
    return res.status(200).json({
      ...
      redirectEndpoint:
        purpose === _purposes.REGISTER ? "/onboarding" : "/app/dashboard",
    });
  }
  ...
  public async refreshToken(
    ...
    const refresh = await this.tokenService.rotateTokens(
    ...
    return res
      .status(200)
      .json({ success: true, message: "Refresh token successful" });
  }
}
